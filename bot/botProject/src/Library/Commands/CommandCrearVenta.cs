...
namespace Ucu.Poo.DiscordBot.Commands
{
    public class CommandCrearVenta : ModuleBase<SocketCommandContext>
    {
        ...
        [Command("crearVenta")]
        public async Task ExecuteAsync(string producto, double monto, DateTime fecha, [Remainder] List<string> datosCliente)
        {
            var clientes = _fachada.BuscarCliente(datosCliente);
            
            if (clientes.Count == 0)
            {
                await ReplyAsync($"No se encontró ningún cliente llamado.");
                return;
            }
